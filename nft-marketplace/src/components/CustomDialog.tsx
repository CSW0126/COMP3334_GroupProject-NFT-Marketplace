    ...
    >
      <div className="relative z-20 flex flex-col rounded-xl bg-white p-4">
        <button onClick={onClose} className="absolute right-4 top-4">
          <XIcon className="h-5 w-5 dark:text-black" />
        </button>
        <Dialog.Title className="text-2xl font-bold dark:text-black">{title}</Dialog.Title>
        <Dialog.Description className="mb-2 dark:text-black">{description}</Dialog.Description>
        {children}
      </div>
